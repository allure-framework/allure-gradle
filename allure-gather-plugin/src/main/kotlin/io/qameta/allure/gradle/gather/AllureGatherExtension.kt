...
import io.qameta.allure.gradle.gather.config.*
...
/**
 * Configures Allure raw result adapters (e.g. [junit5], [testng], ...) as `allure { gather {...` extension.
 */
open class AllureGatherExtension @Inject constructor(
    private val project: Project,
    private val objects: ObjectFactory
) {
    companion object {
        const val NAME = "gather"
    ...
    fun addAspectjTo(task: Task): Unit = task.run {
        if (aspectjWeaver.get() && this is JavaForkOptions) {
            val aspectJAgent = project.configurations[AllureGatherPlugin.ASPECTJ_WEAVER_CONFIGURATION]
            jvmArgumentProviders.add(JavaAgentArgumentProvider(aspectJAgent))
        }
    }

    fun gatherResultsFrom(tasks: TaskCollection<out Task>) {
        project.apply<AllureGatherBasePlugin>()
        tasks.configureEach {
    ...
    // TODO: move to [AllureGatherBasePlugin] like `allure { gatherResults { fromTask(..) } }
    private fun internalGatherResultsFrom(task: Task) {
        task.run {
            ...
            if (this is JavaForkOptions) {
                systemProperty(AllureGatherPlugin.ALLURE_DIR_PROPERTY, rawResults.absolutePath)
            ...
            val allureResults =
                project.configurations[AllureGatherBasePlugin.ALLURE_RAW_RESULT_ELEMENTS_CONFIGURATION_NAME]
